...
class FolderItem:
    ...
    size: Optional[int] = field(default=None, metadata={"description": "Size in bytes (only for files)"})
...
class Folder:
    ...
    items: list[FolderItem] = field(metadata={"description": "List of items in the folder (files and subfolders)"})
...
class ListFolderParams:
    ...
    path: str = field(metadata={"description": "Path of the folder to list; empty string for root"})
...
class DownloadFileURL:
    ...
    url: str = field(metadata={"description": "Pre-signed URL for downloading the specified file"})
